...
static uint16_t phy_xconnect_process(
	...
	for (uint16_t i = 0; i < nb_objs; i++) {
		...
		peer = iface_from_id(iface->bridge_domain);
...
static void phy_xconnect_register(void) {
	register_interface_mode(GR_IFACE_MODE_XCONNECT, "phy_xconnect");
}

static struct gr_node_info info = {
	.node = &xconnect_node,
	.register_callback = phy_xconnect_register,
};
