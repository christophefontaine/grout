...
// Bridge creation request
struct gr_l2_bridge_add_req {
	char name[GR_L2_BRIDGE_NAME_SIZE];
	struct gr_l2_bridge_config config;
};

// Bridge deletion request
...
// Bridge list request (no payload needed)

// Bridge member add request
struct gr_l2_bridge_member_add_req {
	uint16_t bridge_id;
	uint16_t iface_id;
};

// Bridge member delete request
struct gr_l2_bridge_member_del_req {
	uint16_t bridge_id;
	uint16_t iface_id;
};

// Bridge member list request
...
// Bridge config get request
struct gr_l2_bridge_config_get_req {
	uint16_t bridge_id;
};

// Bridge config set request
