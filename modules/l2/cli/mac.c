...
static cmd_status_t mac_add(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	printf("Added %s MAC entry " ETH_F " -> %s in bridge %u\n",
	       req.type == GR_L2_MAC_STATIC ? "static" : "dynamic",
	       &mac,
	       iface_name,
	       bridge_id);
...
static cmd_status_t mac_del(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	printf("Deleted MAC entry " ETH_F " from bridge %u\n", &mac, bridge_id);
...
static cmd_status_t mac_flush(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	printf("Flushed MAC entries from bridge %u", bridge_id);
	if (req.iface_id != 0)
		printf(" interface %s", iface_name);
	if (req.dynamic_only)
		printf(" (dynamic only)");
	printf("\n");
...
static int ctx_init(struct ec_node *root) {
	...
	// MAC table commands
	ret = CLI_COMMAND(
		CLI_CONTEXT(root, CTX_ARG("bridge mac", "Layer 2 bridge configuration.")),
		"add br BRIDGE_ID mac MAC iface IFACE [static]",
		mac_add,
		...
		with_help("Create static entry.", ec_node("str", "static"))
	);
	if (ret < 0)
		return ret;
	...
	ret = CLI_COMMAND(
		...
		with_help(
			"Bridge domain ID (optional).", ec_node_uint("BRIDGE_ID", 1, UINT16_MAX, 10)
		)
	);
	if (ret < 0)
		return ret;
	...
	ret = CLI_COMMAND(
		CLI_CONTEXT(root, CTX_ARG("bridge mac", "Layer 2 bridge configuration.")),
		"flush br BRIDGE_ID [IFACE] [dynamic_only]",
		mac_flush,
		...
		with_help("Flush only dynamic entries.", ec_node("str", "dynamic_only"))
	);
	if (ret < 0)
		return ret;
