...
bridge_list_info(struct gr_api_client *, const struct gr_iface *iface, char *buf, size_t len) {
	...
	snprintf(buf, len, "bridge_id=%u", bridge_info->bridge_id);
}
...
static cmd_status_t bridge_add(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	printf("Created bridge domain %u (%s)\n", resp->bridge_id, resp->name);
	free(resp);
	return CMD_SUCCESS;
}
...
static cmd_status_t bridge_del(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	if (arg_u32(p, "BRIDGE_ID", &bridge_id) < 0) {
		errno = EINVAL;
		return CMD_ERROR;
	}

	req.bridge_id = bridge_id;
...
static cmd_status_t bridge_show_cmd(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	void *bridge_data;
	ret = gr_api_client_send_recv(c, GR_L2_BRIDGE_GET, sizeof(req), &req, &bridge_data);
	bridge = (struct gr_l2_bridge *)bridge_data;
	if (ret < 0)
		return CMD_ERROR;
...
static cmd_status_t bridge_member_add(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	printf("Added interface %s to bridge domain %u\n", iface_name, bridge_id);
	return CMD_SUCCESS;
}
...
static cmd_status_t bridge_member_del(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	printf("Removed interface %s from bridge domain %u\n", iface_name, bridge_id);
	return CMD_SUCCESS;
}

static cmd_status_t bridge_member_list(struct gr_api_client *c, const struct ec_pnode *p) {
	struct gr_l2_bridge_member_list_req req;
...
static cmd_status_t bridge_iface_add(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	free(req);
	resp = resp_ptr;
	printf("Created bridge interface %u\n", resp->iface_id);
	free(resp_ptr);
	return CMD_SUCCESS;
err:
	free(req);
...
static int ctx_init(struct ec_node *root) {
	...
	ret = CLI_COMMAND(
		...
		"bridge NAME bridge_id BRIDGE_ID",
		bridge_iface_add,
		"Create a new bridge interface.",
		with_help("Interface name.", ec_node("any", "NAME")),
		with_help("Bridge domain ID.", ec_node_uint("BRIDGE_ID", 1, UINT16_MAX, 10))
	);
	if (ret < 0)
		return ret;
	...
	ret = CLI_COMMAND(
		CLI_CONTEXT(root, CTX_ARG("bridge", "Layer 2 bridge configuration.")),
		"member list BRIDGE_ID",
		bridge_member_list,
		"List bridge domain members.",
		with_help("Bridge domain ID.", ec_node_uint("BRIDGE_ID", 1, UINT16_MAX, 10))
	);
	if (ret < 0)
		return ret;
